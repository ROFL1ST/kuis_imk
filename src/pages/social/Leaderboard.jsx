...
const Leaderboard = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Link to={`/topic/${slug}`} className="inline-flex items-center gap-2 text-slate-500 hover:text-indigo-600 mb-6">
        <ArrowLeft size={18} /> Kembali ke Kuis
      </Link>

      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-slate-800 mb-2 capitalize">Top 10 {slug}</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          {leaders.map((entry, index) => (
            <div 
              key={index}
              className={`p-4 rounded-xl border flex items-center justify-between shadow-sm ${getRankStyle(index)}`}
            >
              <div className="flex items-center gap-4">
                <div className="font-bold text-xl w-8 text-center">#{index + 1}</div>
                <div>
                  <h3 className="font-bold">{entry.name}</h3>
                  <p className="text-xs opacity-75">@{entry.username}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Medal size={18} />
                <span className="font-bold text-lg">{entry.total_points} Poin</span>
              </div>
            </div>
          ))}
