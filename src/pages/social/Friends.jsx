  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <form onSubmit={handleAddFriend} className="flex w-full md:w-auto relative group">
           <UserPlus className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition" size={18} />
           <input 
             type="text" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* === LEFT COLUMN: DAFTAR TEMAN === */}
        <div className="lg:col-span-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              <User className="text-indigo-600" /> Daftar Teman <span className="text-slate-400 text-sm font-normal">({friends.length})</span>
            </h2>
          </div>
          
          {loading ? (
             <div className="text-center py-12 text-slate-500">Memuat teman...</div>
          ) : friends.length === 0 ? (
             <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
                <div className="w-16 h-16 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-4">
                   <User size={32} />
                </div>
                <p className="text-slate-500">Belum ada teman. Tambahkan seseorang!</p>
             </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {friends.map((friend) => (
                <div key={friend.ID} className="relative bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all group overflow-visible z-0 hover:z-10">
        ...
        <div className="lg:col-span-4 space-y-6">
           ...
           <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
              <h2 className="font-bold text-slate-800 mb-4 flex items-center justify-between">
                 <span className="flex items-center gap-2"><Bell className="text-amber-500" size={18}/> Permintaan Masuk</span>
                 {requests.length > 0 && (
                    <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full font-bold animate-pulse">{requests.length}</span>
                 )}
              </h2>
              ...
              {requests.length === 0 ? (
                 ...
                 <div className="space-y-3">
                    {requests.map((req) => (
                       <div key={req.ID} className="p-3 bg-slate-50 rounded-xl border border-slate-100 transition hover:border-indigo-100 hover:shadow-sm">
                          <div className="flex items-center gap-3 overflow-hidden mb-3">
                             ...
                             <div className="min-w-0">
                                <p className="font-bold text-sm text-slate-800 truncate">{req.requester?.name}</p>
                                <p className="text-xs text-slate-500 truncate">@{req.requester?.username}</p>
                             </div>
                          </div>
           ...
           {sentRequests.length > 0 && (
             <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
               <h2 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                 <Clock className="text-blue-500" size={18} /> Menunggu Konfirmasi
               </h2>
               <div className="space-y-3">
                 {sentRequests.map((req) => (
                   <div key={req.ID} className="flex items-center justify-between p-3 bg-blue-50/50 rounded-xl border border-blue-100">
                     <div className="flex items-center gap-3 overflow-hidden">
                       ...
                       <div className="min-w-0">
                         <p className="font-bold text-xs text-slate-800 truncate">{req.friend?.name}</p>
                         <p className="text-[10px] text-slate-500 truncate">@{req.friend?.username}</p>
                       </div>
                     </div>
                     <button 
      ...
      <Modal isOpen={confirmModal.isOpen} onClose={() => setConfirmModal({isOpen: false, friendId: null, friendName: ""})} maxWidth="max-w-sm">
         <div className="text-center">
            <div className="w-12 h-12 bg-red-100 text-red-500 rounded-full flex items-center justify-center mx-auto mb-3">
               <Trash2 size={24} />
            </div>
            <h3 className="text-lg font-bold text-slate-800 mb-2">Hapus Teman?</h3>
            <p className="text-slate-600 text-sm mb-6 leading-relaxed">
               Apakah Anda yakin ingin menghapus <span className="font-bold text-slate-800">{confirmModal.friendName}</span> dari daftar teman?
            </p>
            <div className="flex gap-3">
               <button onClick={() => setConfirmModal({isOpen: false, friendId: null, friendName: ""})} className="flex-1 py-2.5 border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 transition">
                  Batal
               </button>
               <button onClick={executeRemoveFriend} className="flex-1 py-2.5 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition">
                  Ya, Hapus
               </button>
            </div>
         </div>
