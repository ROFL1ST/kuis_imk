...
const QuizPlay = () => {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-medium text-slate-500">Memuat Soal...</div>;
      ...
      <div className="min-h-screen bg-slate-50 py-12 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <h2 className="text-slate-500 font-medium uppercase tracking-widest mb-4">Hasil Kuis</h2>
            
            <div className="flex justify-center items-center mb-6">
              <div className={`relative w-40 h-40 rounded-full flex items-center justify-center border-[6px] 
                ${isPass ? 'border-green-100 bg-green-50 text-green-600' : 'border-red-100 bg-red-50 text-red-500'}
              `}>
                <div className="text-center">
                  <span className="block text-5xl font-black">{resultData.score}</span>
                  <span className="text-sm font-bold opacity-70">POIN</span>
                </div>
                {isPass && <Trophy className="absolute -top-2 -right-2 text-yellow-400 fill-current drop-shadow-md" size={48} />}
              </div>
            </div>

            <h1 className={`text-2xl font-bold mb-2 ${isPass ? 'text-green-700' : 'text-red-600'}`}>
            ...
            <div className="grid grid-cols-3 gap-4 border-t pt-6 mt-6">
              ...
              <div className="text-center border-l border-r border-slate-100">
                <p className="text-xs text-slate-400 font-bold uppercase mb-1">Benar</p>
                <p className="font-bold text-green-600 flex justify-center items-center gap-1">
                  <CheckCircle2 size={16} /> {resultData.correct}
                </p>
              </div>
              <div className="text-center">
                <p className="text-xs text-slate-400 font-bold uppercase mb-1">Salah</p>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-4">
              {questions.map((q, idx) => {
                ...
                return (
                  <div key={q.ID} className={`bg-white p-5 rounded-xl border-l-4 shadow-sm ${isCorrect ? 'border-l-green-500' : 'border-l-red-500'}`}>
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className={`flex items-center gap-2 p-2 rounded-lg ${isCorrect ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                            {isCorrect ? <CheckCircle2 size={16}/> : <XCircle size={16}/>}
                            <span className="font-semibold">Jawabanmu: {myAnswer || "-"}</span>
                          </div>
                          {!isCorrect && (
                            <div className="flex items-center gap-2 p-2 rounded-lg bg-slate-100 text-slate-600">
                              <CheckCircle2 size={16} className="text-green-600"/>
                              <span>Kunci: <span className="font-semibold text-slate-800">{q.correct}</span></span>
                            </div>
                          )}
                        </div>
                      </div>
          ...
          <div className="flex gap-3 justify-center pb-8">
            ...
            <Link to="/history" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 shadow-md transition">
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-3xl">
        ...
        <div className="mt-8 flex justify-between">
          ...
          >
            {submitting ? "Menghitung..." : currentIndex === questions.length - 1 ? "Selesai" : "Lanjut"}
          </button>
        </div>
