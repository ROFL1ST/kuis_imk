...
import { 
  CheckCircle2, XCircle, ArrowLeft, Trophy, Swords 
} from "lucide-react";
...
const ReviewPage = () => {
  ...
  if (loading) return <div className="text-center py-20 text-slate-500">Memuat detail...</div>;
  if (!detail) return <div className="text-center py-20">Data tidak ditemukan</div>;
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      <Link to="/history" className="inline-flex items-center gap-2 text-slate-500 hover:text-indigo-600 mb-6 font-medium">
        <ArrowLeft size={18} /> Kembali ke Riwayat
      </Link>

      {/* HEADER CARD SUMMARY */}
      <div className={`bg-white rounded-3xl shadow-sm border overflow-hidden mb-8 relative p-8 text-center
        ${isPass ? 'border-green-200' : 'border-red-200'}
      `}>
         <div className={`absolute top-0 left-0 w-full h-1.5 ${isPass ? 'bg-green-500' : 'bg-red-500'}`}></div>
         
         <h2 className="text-slate-400 text-sm font-bold uppercase tracking-wider mb-4 flex justify-center items-center gap-2">
            {isDuel ? <><Swords size={16}/> Duel Report</> : "Quiz Report"}
         </h2>
         ...
         <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto">
            <div className="bg-green-50 rounded-xl p-3 border border-green-100">
               <div className="text-xs text-green-600 font-bold uppercase">Benar</div>
               <div className="text-xl font-black text-green-700">{correctCount}</div>
            </div>
            <div className="bg-red-50 rounded-xl p-3 border border-red-100">
               <div className="text-xs text-red-600 font-bold uppercase">Salah</div>
      ...
      {/* QUESTION LIST */}
      <h3 className="text-lg font-bold text-slate-700 mb-4 px-2">Detail Jawaban</h3>
          ...
          return (
            <div key={q.ID} className={`bg-white p-5 rounded-xl border-l-4 shadow-sm ${isCorrect ? 'border-l-green-500' : 'border-l-red-500'}`}>
              <div className="flex gap-4">
                 <div className="text-slate-400 font-bold text-sm pt-1">#{index+1}</div>
                    ...
                    <div className="space-y-2">
                       ...
                       `}>
                          <span className="text-sm font-medium">
                            <span className="opacity-70 text-xs uppercase mr-2">Jawabanmu:</span> 
                       ...
                       {!isCorrect && (
                         <div className="flex items-center gap-2 p-3 rounded-lg bg-slate-50 border border-slate-200 text-slate-600">
                            ...
                            <span className="text-sm">
                               <span className="opacity-70 text-xs uppercase mr-2">Kunci:</span>
