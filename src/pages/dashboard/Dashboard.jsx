...
const Dashboard = () => {
  ...
  if (loading) return <div className="text-center py-20">Memuat Dashboard...</div>;
    ...
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2">
        <div className="mb-6">
          ...
          <p className="text-slate-500 mt-1">Pilih mata kuliah untuk memulai kuis.</p>
        </div>
      ...
      <div className="lg:col-span-1">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 h-fit sticky top-24">
          ...
          <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {activities.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">Belum ada aktivitas terbaru.</p>
            ) : (
              activities.map((act) => (
                <div key={act.ID} className="flex gap-3 items-start border-b border-slate-50 pb-3 last:border-0">
                  ...
                  <div>
                    <p className="text-sm text-slate-700">
                      <span className="font-bold">{act.user.name}</span> {act.description}
                    </p>
                    <p className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                      <Zap size={10} /> {act.type.replace('_', ' ').toUpperCase()}
                    </p>
                  </div>
                </div>
              ))
