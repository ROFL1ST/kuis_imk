...
import {
  User, Mail, Lock, Edit3, Trophy, Target,
  BarChart2, Award, BookOpen, Calendar,
  CheckCircle, Zap, Star, TrendingUp,
  Crown, Clock, Shield, ChevronRight, Sparkles,
    ...
    >
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                >
                  ...
                  <span className="font-bold text-yellow-700">{user.xp.toLocaleString()} XP</span>
                </motion.div>
              ...
              <div className="mt-8">
                <div className="flex justify-between text-sm mb-2">
                  <span className="font-medium text-slate-600">Progress Level {currentLevel}</span>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-slate-500 text-sm">Analisis kemampuan berdasarkan topik</p>
            </div>
          ...
          <div className="space-y-6">
            {topic_performance.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-400">Mulai kerjakan kuis untuk melihat performa!</p>
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-bold text-slate-700">{topic.topic_name}</span>
                        <p className="text-xs text-slate-500">{topic.total_quizzes} kuis dikerjakan</p>
                      </div>
                    </div>
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
              {achievements.length > 0 ? (
                achievements.map((ach) => (
                  <AchievementItem
                    key={ach.id}
                    icon={<span className="text-xl leading-none">{ach.icon_url}</span>} // Menggunakan Emoji dari DB
                    title={ach.name}
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <InfoItem
                ...
                value={user.last_activity_date 
                  ? new Date(user.last_activity_date).toLocaleString('id-ID', {
                      day: 'numeric',
                      month: 'short',
                      hour: '2-digit',
                      minute: '2-digit'
      ...
      >
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-2xl font-bold mb-2">Terus tingkatkan kemampuanmu! 🚀</h3>
            <p className="text-indigo-100">
              {stats.total_quizzes > 0 
                ? `Kamu sudah menyelesaikan ${stats.total_quizzes} kuis dengan rata-rata skor ${Math.round(stats.average_score)}%. Pertahankan semangat belajar!`
                : `Mulai petualangan kuis pertamamu dan dapatkan XP!`
              }
            </p>
          </div>
          <button className="px-6 py-3 bg-white text-indigo-600 font-bold rounded-xl hover:bg-indigo-50 transition-all duration-300 flex items-center gap-2 shadow-lg hover:shadow-xl">
            <Sparkles size={18} />
            Kerjakan Kuis Baru
...
const StatCard = ({ icon, label, value, change, color, delay, isText = false }) => {
  const colorClasses = {
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className={`font-bold text-slate-900 ${isText ? 'text-xl' : 'text-3xl'}`}>
          {value}
        </p>
      </div>
    </motion.div>
...
const AchievementItem = ({ icon, title, description, unlocked }) => (
  ...
  `}>
    ...
    <div className="flex-1">
      <p className={`font-bold ${unlocked ? 'text-slate-800' : 'text-slate-500'}`}>{title}</p>
...
const InfoItem = ({ icon, label, value, isComponent = false }) => (
  <div className="flex items-center justify-between py-3 border-b border-slate-100 last:border-0">
    ...
    {isComponent ? value : <span className="font-medium text-slate-800">{value}</span>}
  </div>
);
