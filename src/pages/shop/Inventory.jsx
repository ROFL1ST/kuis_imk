    ...
    if (item.type === 'title') {
        return (
            <div className="flex flex-col items-center justify-center h-full w-full gap-1">
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Preview</span>
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="text-center w-full">
                    ...
                    <div className="min-h-[2rem]">
                        {user.equipped_items?.find((i) => i.item.type === "title") ? (
                            ...
                            <span className="text-xs text-slate-300 italic">Tidak ada gelar dipakai</span>
                        )}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 w-full mt-6 pt-6 border-t border-slate-100">
                    <div className="text-center p-2 rounded-xl bg-slate-50">
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Frame</p>
                        <p className="text-xs font-bold text-slate-700 truncate px-1">
                            {user.equipped_items?.find((i) => i.item.type === "avatar_frame")?.item.name || "-"}
                        </p>
                    </div>
                    <div className="text-center p-2 rounded-xl bg-slate-50">
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Total Item</p>
                        <p className="text-xs font-bold text-slate-700">{inventory.length}</p>
                    </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="lg:col-span-8">
            <div className="hidden lg:block mb-8">
                <h1 className="text-3xl font-black text-slate-800 flex items-center gap-3">
                   <span className="bg-indigo-100 p-2 rounded-xl text-indigo-600"><Package size={28} /></span> 
                   Koleksi Item
                </h1>
                <p className="text-slate-500 ml-14">Kelola dan gunakan item yang sudah kamu beli.</p>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {[
                ...
                { id: "avatar_frame", label: "Bingkai", icon: <Layout size={14} /> },
                ...
                <AnimatePresence mode="popLayout">
                    {filteredInventory.length === 0 ? (
                        ...
                        >
                            ...
                            <h3 className="text-lg font-bold text-slate-700">Tas Masih Kosong</h3>
                        ...
                        {filteredInventory.map((inv) => (
                            ...
                            >
                                ...
                                {/* AREA PREVIEW ITEM */}
                                <div className={`w-24 h-24 rounded-2xl flex items-center justify-center mb-1 transition-transform group-hover:scale-105 overflow-hidden ${
                                ...
                                <div className="w-full">
                                    <h3 className={`font-bold text-sm truncate ${inv.is_equipped ? "text-indigo-700" : "text-slate-700"}`}>
                                        {inv.item.name}
                                    </h3>
